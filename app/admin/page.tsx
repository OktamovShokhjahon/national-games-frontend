...
import { FiLogIn, FiLogOut, FiPlus, FiEdit, FiTrash2, FiUpload, FiX } from 'react-icons/fi';
                  ...
                  <form onSubmit={handleGameSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {(['uz', 'en', 'ru'] as const).map((lang) => (
                        <div key={lang}>
                          <label className="block text-sm font-medium mb-2">Name ({lang.toUpperCase()})</label>
                          <input
                            type="text"
                            value={gameForm.name[lang]}
                            onChange={(e) => setGameForm({ ...gameForm, name: { ...gameForm.name, [lang]: e.target.value } })}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {(['uz', 'en', 'ru'] as const).map((lang) => (
                        <div key={lang}>
                          <label className="block text-sm font-medium mb-2">Short Description ({lang.toUpperCase()})</label>
                          <textarea
                            value={gameForm.shortDescription[lang]}
                            onChange={(e) => setGameForm({ ...gameForm, shortDescription: { ...gameForm.shortDescription, [lang]: e.target.value } })}
                            className="w-full px-4 py-2 rounded-lg border bg-white dark:bg-gray-800"
                            rows={3}
                            required
                          />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {(['uz', 'en', 'ru'] as const).map((lang) => (
                        <div key={lang}>
                          <label className="block text-sm font-medium mb-2">Description ({lang.toUpperCase()})</label>
                          <textarea
                            value={gameForm.description[lang]}
                            onChange={(e) => setGameForm({ ...gameForm, description: { ...gameForm.description, [lang]: e.target.value } })}
                            className="w-full px-4 py-2 rounded-lg border bg-white dark:bg-gray-800"
                            rows={5}
                            required
                          />
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">Images (Multiple images allowed)</label>
                      <input
                        type="file"
                        multiple
                        accept="image/*"
                        onChange={(e) => {
                          ...
                          setGameForm({ ...gameForm, images: [...gameForm.images, ...files] });
                        }}
                          ...
                          {gameForm.images.map((image, index) => (
                            <div key={index} className="relative group">
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  const newImages = gameForm.images.filter((_, i) => i !== index);
                                  setGameForm({ ...gameForm, images: newImages });
                                }}
                      ...
                      {editingGame && editingGame.images && editingGame.images.length > 0 && (
                        <div className="mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {games.map((game) => (
                  <div key={game._id} className="glass rounded-2xl p-6">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{game.name.uz}</h3>
                    <p className="text-gray-600 dark:text-gray-400 mb-4 line-clamp-2">{game.shortDescription.uz}</p>
